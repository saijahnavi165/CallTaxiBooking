/*
The are 6 points(A,B,C,D,E,F) 15 KM apart 15 min(I have done it with an hour) travel between each, n taxis all taxis at A starting
50 rs for first 5 KM and then 10 for each of the further KMs, rate from pickup to drop only
pickup time example : 9 hrs, 15 hrs
(As it makes complicated by addng 15 minutes i have added time to be an hour)
...
public class Booking
{
    public static void bookTaxi(int customerID,char pickupPoint,char dropPoint,int pickupTime,List<Taxi> freeTaxis)
    {
        ...
        for(Taxi t : freeTaxis)
        {
            ...
            if(distanceBetweenCustomerAndTaxi < min)
            {
                ...
                //trip earning = 50 + (distanceBetweenpickUpandDrop-5) * 10
                earning = (distanceBetweenpickUpandDrop-5) * 10 + 50;
                //(As it makes complicated by addng 15 minutes I have added time to be an hour)
    ...
    public static void main(String[] args)
    {
        ...
        while(true)
        {
        System.out.println("Enter 1 for Booking Taxi");
        System.out.println("Enter 2 to Print Taxi details");
        ...
        if(pickupPoint < 'A' || dropPoint > 'E' || pickupPoint > 'E' || dropPoint < 'A')
        {
            System.out.println("Valid pickup and drop are A, B, C, D, E. Exitting");
            return;
        }
